...
void setEXTLED(int onOff) {
	   if (onOff == ON) {
					PTD->PSOR = MASK(EXT_LED_POS);
		 }
		 if (onOff == OFF) {
        PTD->PCOR = MASK(EXT_LED_POS) ;
    }
    // no change otherwise
}
...
void configureOutput() {
     ...
     PORTD->PCR[EXT_LED_POS] &= ~PORT_PCR_MUX_MASK;	
     PORTD->PCR[EXT_LED_POS] |= PORT_PCR_MUX(1);   

     // Set ports to outputs
     PTB->PDDR |= MASK(RED_LED_POS) | MASK(GREEN_LED_POS);
     PTD->PDDR |= MASK(BLUE_LED_POS)| MASK(EXT_LED_POS);
